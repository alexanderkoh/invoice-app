...
export async function generateTemplate1(data: InvoiceType) {
    ...
    // Draw sender name in blue (positioned to not overlap with logo)
    const senderNameY = LETTER_HEIGHT - margin - (logoSpace.height > 0 ? logoSpace.reservedHeight : 20);
    ...
    // Draw invoice header on the right (aligned to the top)
    const invoiceText = 'Invoice #';
    const invoiceNumberWidth = boldFont.widthOfTextAtSize(data.details.invoiceNumber, 12);
    const invoiceTextWidth = boldFont.widthOfTextAtSize(invoiceText, 24);
    
    page.drawText(invoiceText, {
        x: LETTER_WIDTH - margin - invoiceTextWidth,
    ...
    page.drawText(data.details.invoiceNumber, {
        x: LETTER_WIDTH - margin - invoiceNumberWidth,
        y: LETTER_HEIGHT - margin - 25,
        size: 12,
        font: font,
        color: rgb(0.5, 0.5, 0.5),
    });
    
    // Draw sender address (positioned below invoice number)
    let addressY = LETTER_HEIGHT - margin - 50; // Start below invoice number
    const addressLines = [
        data.sender.address,
    ...
    addressLines.forEach(line => {
        addressY -= lineHeight;
        page.drawText(line, {
            x: LETTER_WIDTH - margin - font.widthOfTextAtSize(line, 12),
            y: addressY,
            size: 12,
            font: font,
            color: rgb(0.3, 0.3, 0.3),
    ...
    // Draw dates
    let datesY = yOffset + lineHeight * 3;
    const dateLabels = [
